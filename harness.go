...
// isRuntimeReady 校验 Tavern 源码、Profile 与独立 DSH 入口是否就绪
func isRuntimeReady() bool {
	if fi, err := os.Stat(tavernSourceDir); err != nil || !fi.IsDir() {
		return false
	}
	for _, path := range []string{
		filepath.Join(tavernSourceDir, "package.json"),
		filepath.Join(tavernSourceDir, "node_modules", "yaml", "package.json"),
		filepath.Join(globalDshHome, "profiles", "tavern", "package.json"),
		filepath.Join(runtimeDir, "bin", "dsh"),
		filepath.Join(runtimeDir, "lib", "node_modules", "@deepseek-ai", "dsh", "lib", "bin.js"),
	} {
...
func EvaluateDeploymentPolicy(tarPath string) (shouldDeploy bool, isUpgrade bool, reason string) {
	...
	return false, false, fmt.Sprintf("Tavern 运行环境已就绪 (v%s)，跳过内置源码部署", installedVer)
}
...
func deployBuiltinPackage(tarPath, zipVer string, isUpgrade bool) {
	...
	go func() {
		...
		_ = safeRemoveAll(tavernSourceDir)

		if err := extractTarGz(tarPath, tavernSourceDir); err != nil {
		...
		if err := installTavern(false); err != nil {
			LogWarning("Tavern Profile 安装失败: %s", err)
		...
		LogInfo("Tavern 运行环境部署完成，正在启动服务")
...
func InitHarness() {
	...
	tarPath := filepath.Join(globalAppDest, "dsh-tavern.tar.gz")
	zipVer := readAppDestVersion()

	if _, err := os.Stat(tarPath); err == nil && !isRuntimeReady() {
		LogInfo("正在部署内置 DSH Tavern 源码包 (v%s)", zipVer)
		deployBuiltinPackage(tarPath, zipVer, false)
		return
	}
	if !isRuntimeReady() {
		state.SetStatus(StatusBuilding, "正在安装 DSH Tavern 运行环境...")
		LogInfo("未检测到完整运行环境，通过 Tavern 安装器初始化")
		go func() {
			if err := installTavern(false); err != nil {
				LogWarning("Tavern 运行环境安装失败: %s", err)
				state.SetStatus(StatusStopped, "安装失败: "+err.Error())
				return
			}
			refreshVersion()
			state.SetStatus(StatusStopped, "")
			LogInfo("Tavern 运行时部署完成，正在启动服务")
			if err := Start(); err != nil {
				LogWarning("服务启动失败: %s", err)
			}
		}()
...
func dshCliCmd(subArgs ...string) (string, []string) {
	...
	cliBinJs := filepath.Join(runtimeDir, "lib", "node_modules", "@deepseek-ai", "dsh", "lib", "bin.js")
	if _, err := os.Stat(cliBinJs); err == nil {
		return nodeBin(), append(append(v8Args, cliBinJs), subArgs...)
	}

	binLink := filepath.Join(runtimeDir, "bin", "dsh")
...
func startLocked() error {
	...
	// Always launch the Tavern profile. Without this flag DSH falls back to the
	// default web profile, so the Tavern client bundles are never served.
	bin, args := dshCliCmd("--profile", tavernProfileName, "--host", "127.0.0.1", "--port", fmt.Sprintf("%d", port), "--no-open")
