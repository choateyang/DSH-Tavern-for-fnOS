...
func pidFilePath() string {
	return filepath.Join(globalPkgVar, "dsh-tavern.pid")
}
...
func isDshProcess(pid int) bool {
	...
	if cwd, err := os.Readlink(fmt.Sprintf("/proc/%d/cwd", pid)); err == nil && (strings.HasPrefix(cwd, runtimeDir) || strings.HasPrefix(cwd, tavernSourceDir)) {
		return true
	}
	if data, err := os.ReadFile(fmt.Sprintf("/proc/%d/cmdline", pid)); err == nil {
		cmdline := string(data)
		return strings.Contains(cmdline, runtimeDir) || strings.Contains(cmdline, tavernSourceDir) || strings.Contains(cmdline, "@deepseek-ai/dsh") || strings.Contains(cmdline, "dsh-tavern")
	}
	return false
}
