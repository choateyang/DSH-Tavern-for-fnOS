# DSH Tavern 反向代理与网关适配技术文档

本文档记录了在飞牛 NAS 系统中，针对 **DSH Tavern** 进行反向代理与网关子路径适配时所做的特殊处理与技术实现，供后续维护与版本升级参考。
...
- **全场景适配**：
  1. **飞牛网关模式**：子路径代理（`http://<NAS_IP>:5666/app/dsh-tavern/fngateway/`）；
  2. **独立代理模式**：独立端口（`https://<NAS_IP>:3082/`）；
  3. **本地回环模式**：本地调试（`http://127.0.0.1:3081/`）。
