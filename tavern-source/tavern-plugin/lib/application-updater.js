...
const FNOS_UPDATE_MESSAGE = 'DSH Tavern 更新由 fnOS 管理器统一执行，请返回管理器的“检查更新”；此处不更新独立 DSH 运行时。'
...
export function createApplicationUpdater(options) {
  ...
  const managedByFnos = process.env.DSH_TAVERN_FNOS === '1' && process.env.DSH_TAVERN_FNOS_MANAGER_UPDATE !== '1'
  ...
  async function writeStatus(value) {
    markManagedStatus(value)
    await store.writeJson(STATUS_FILE, value)
    record('status', value)
    return value
  }
  function markManagedStatus(value) {
    if (managedByFnos && value && typeof value === 'object') {
      value.managedByFnos = true
      value.managerMessage = FNOS_UPDATE_MESSAGE
    }
    return value
  }
  ...
  async function versions(identity) {
    ...
    try {
      const [remote, latestCommitResult] = await Promise.all([
        stage('github.version', fetchManifest),
        stage('github.commit', fetchLatestCommit),
      ])
      ...
      const normalizedCurrentCommit = currentCommit.toLowerCase() === publishedCommit.toLowerCase()
        ...
        : currentCommit
      let updateAvailable
      if (/^[0-9a-f]{40}$/i.test(normalizedCurrentCommit)) {
        ...
        throw new Error('当前构建缺少有效提交号，需手动重新安装')
      }
      return {
      ...
      record('fallback.jsdelivr', { reason: String(githubError?.message || githubError) })
      try {
        ...
        if (!compared.matches && currentCommit.toLowerCase() !== String(compared.revision).toLowerCase()) {
          if (currentReleaseSequence && compared.releaseSequence) {
            ...
            throw new Error('jsDelivr 清单缺少可比较的发布序号')
          }
        }
        return {
          currentVersion,
        ...
        throw new Error(`GitHub 不可用（${sanitizeUpdateError(githubError?.message || githubError)}）；jsDelivr 备用源也不可用（${sanitizeUpdateError(cdnError?.message || cdnError)}）`)
      }
    }
  }
  ...
  async function status() {
    return markManagedStatus(await statusWithIdentity(await localIdentity()))
  }
  ...
  async function start() {
    ...
    if (managedByFnos) {
      const managed = {
        phase: 'managed-by-fnos', host: installHost, checkedAt: now(),
        currentVersion: current.currentVersion, currentCommit: current.currentCommit,
        error: FNOS_UPDATE_MESSAGE,
      }
      await writeStatus(managed)
      return managed
    }
    let version
    try {
