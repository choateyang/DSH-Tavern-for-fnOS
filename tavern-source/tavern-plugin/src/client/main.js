	...
	factory: (require) => {
		...
		function useCardBatchDeletion(cards, busy, setBusy, refresh) {
			...
			function toolbar(visible) {
				...
				return h("div", { className: "dsh-tavern-card-batch" },
					h("button", { className: "dsh-tavern-btn", disabled: busy, onClick: function () { setManaging(!managing); setPaths([]); setNotice(""); } }, managing ? "取消批量选择" : "批量删除"),
			...
			return { managing, toggle, toolbar, checkbox, isSelected, reset, paths };
		}
		...
		function TavernSidebar(props) {
			...
			async function performUpdate() {
				if (updateStatus.phase !== "update-available" || updateStatus.managedByFnos) return;
			...
			const playPicker = h("div", { className: "dsh-tavern-card-picker", role: "dialog", "aria-modal": "true", "aria-label": openingPicker ? "游戏准备" : "选择人物卡开始游玩" }, pickerError, openingPicker ? openingChoice : h(React.Fragment, null,
				...
				cards.length ? h(React.Fragment, null, h("div", { className: "dsh-tavern-side-empty", style: { padding: "4px 6px" } }, "已绑定剧本的人物卡将自动按剧本推进；未绑定的按自由故事推进。剧本绑定在“卡片模式”中管理。"), cards.map(function (card) { return h("div", { key: card.path, className: "dsh-tavern-card-pick-wrap" },
					...
					!cardBatch.managing ? h("button", { className: "dsh-tavern-script-file", disabled: busy, title: "从人物卡库删除", onClick: function () { if (window.confirm("从人物卡库删除“" + card.name + "”吗？\n人物卡工作版和原版都会删除，已有对话会保留。")) call("deleteCard", { path: card.path }).then(refresh, function (err) { setError(String(err && err.message || err)); }); } }, "删除") : null,
					!cardBatch.managing ? h("button", { className: "dsh-tavern-script-file", disabled: busy, title: "导出为 SillyTavern 兼容 JSON", onClick: function () { exportCard(card); } }, "导出") : null
			...
			const fnosManaged = updateStatus.managedByFnos === true;
			const updateMessage = fnosManaged && updateStatus.phase === "update-available"
				? "发现 DSH Tavern 新提交 " + ((updateStatus.latestCommit || "").slice(0, 8) || updateStatus.latestVersion || "") + "。请返回 fnOS 的 DSH Tavern 管理器，在“检查更新”中操作；此处不更新独立 DSH 运行时。"
			: fnosManaged && updateStatus.phase === "managed-by-fnos"
				? "DSH Tavern 更新由 fnOS 管理器统一执行，请返回管理器操作。此处不更新独立 DSH 运行时。"
			: updateStatus.phase === "checking"
				? "正在通过 GitHub 检查 DSH Tavern 最新提交…"
				: updateStatus.phase === "up-to-date"
					? "✓ DSH Tavern 已是最新提交"
				: updateStatus.phase === "update-available"
					? "发现 DSH Tavern 新提交 " + ((updateStatus.latestCommit || "").slice(0, 8) || updateStatus.latestVersion || "")
			...
			const currentCommitLabel = (updateStatus.currentCommit || "").slice(0, 8) || "构建未知";
			const updateHostLabel = fnosManaged ? "fnOS 托管版" : (updateStatus.host === "desktop" ? "Desktop 版" : (updateStatus.host === "android" ? "Android 版" : "命令行版"));
			const checkingOrRunning = updateStatus.phase === "checking" || updateStatus.phase === "running" || updateStatus.phase === "loading";
			const updateActions = fnosManaged && updateStatus.phase === "update-available"
				? h("div", { className: "dsh-tavern-update-actions" },
					h("button", { className: "dsh-tavern-update-button primary", disabled: true }, "请在 fnOS 管理器更新"))
				: updateStatus.phase === "update-available"
				? h("div", { className: "dsh-tavern-update-actions" },
		...
		function UserPreferenceProfileTab(props) {
			...
			const controls = record ? h("div", { className: "dsh-tavern-profile-controls" },
				currentConversation ? h("section", { className: "dsh-tavern-profile-game", "aria-label": "当前游戏画像" },
			...
			function ExternalPresetLibraryTab(props) {
				...
				if (preset && preset.path === detailPath) {
					...
					h("div", { className: "dsh-tavern-preset-detail" }, error ? h("div", { className: "dsh-tavern-dock-error" }, error) : null,
						...
						h("div", { className: "dsh-tavern-preset-detail-actions" }, isPlayMode(catalog.sessionMode) ? h("button", { className: "dsh-tavern-btn", disabled: busy || Object.keys(entryDrafts).length > 0 || Object.keys(regexDrafts).length > 0, title: "请先保存条目和正则的修改", onClick: function () { applyConversationPreset(preset.path); } }, "应用到当前游戏") : null, h("button", { className: "dsh-tavern-btn", disabled: busy, onClick: function () { exportFile(preset); } }, "导出"), h("button", { className: "dsh-tavern-btn", disabled: busy, onClick: function () { rename(preset); } }, "重命名"), h("button", { className: "dsh-tavern-btn danger", disabled: busy, onClick: function () { remove(preset); } }, "删除")),
