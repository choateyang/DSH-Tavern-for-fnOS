...
describe('image tool registration', () => {
  it('Tavern deployment keeps settings and Studio but does not expose paid tools to Agents', () => {
    const { ctx, tools, installSection } = harnessContext()
    apply(ctx, { registerAgentTools: false })
    expect(tools).toEqual([])
    expect(installSection).toHaveBeenCalledTimes(1)
    expect(ctx.webServer.register).toHaveBeenCalledWith(expect.objectContaining({ path: '/plugins/dsh-image-gen/studio' }))
  })
  beforeEach(() => { vi.clearAllMocks() })
  afterEach(() => { vi.unstubAllGlobals() })

  it('runs Grok through the registered tool and rejects editing before accessing references', async () => {
    const { ctx, tools } = harnessContext()
    apply(ctx, { provider: 'grok', saveToWorkspace: false })
    const png = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+jRZkAAAAASUVORK5CYII='
    const request = vi.fn(async () => Response.json({ data: [{ b64_json: png }] }))
    vi.stubGlobal('fetch', request)
    const exec = { signal: new AbortController().signal } as never
    const result = await toolByName(tools, 'generate_image').execute({ prompt: 'a lake', resolution: '2k', aspect_ratio: '16:9' }, exec)
    expect(result).toMatchObject({ provider: 'grok', output: '16:9, 2k' })
    await expect(toolByName(tools, 'edit_image').execute({ prompt: 'a lake' }, exec)).rejects.toThrow('仅支持文生图')
