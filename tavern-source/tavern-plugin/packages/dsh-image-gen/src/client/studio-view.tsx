    ...
    <section className="dsh-ig-workbench" aria-label={t('title')}>
      <div className={`dsh-ig-workbench-grid ${sidebarCollapsed ? 'is-sidebar-collapsed' : ''}`}>
        ...
        <main className="dsh-ig-canvas-column">
          ...
          {selected !== null && <>
            <div className="dsh-ig-result-strip"><span>{t('result')}</span><div><button type="button" disabled={config?.providers.find(item => item.provider === selected.provider)?.supportsEditing !== true} onClick={() => void continueEdit()}><PencilLine size={15} />{t('continueEdit')}</button><button type="button" onClick={() => { setMode('generate'); setPanelTab('generate'); setPrompt(selected.prompt) }}><RefreshCw size={15} />{t('regenerate')}</button></div></div>
        ...
        <aside className="dsh-ig-generate-panel">
          ...
          {panelTab === 'details' ? <DetailsPanel item={selected} t={t} /> : <div className="dsh-ig-generator-form">
            <div className="dsh-ig-mode-switch"><button type="button" className={mode === 'generate' ? 'is-active' : ''} onClick={() => setMode('generate')}><Sparkles size={15} />{t('generate')}</button><button type="button" disabled={activeProfile?.supportsEditing !== true} className={mode === 'edit' ? 'is-active' : ''} onClick={() => setMode('edit')}><ImagePlus size={15} />{t('edit')}</button></div>
            {mode === 'edit' && (
