...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  // This is a deployment setting, not a per-user settings toggle. Keep Studio
  // and configuration available while Tavern owns paid-generation initiation.
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      aspect_ratio: { type: 'string', enum: ['1:1', '3:2', '2:3', '4:3', '3:4', '16:9', '9:16'], description: 'Optional output aspect ratio for Google Gemini or Grok.' },
      resolution: { type: 'string', enum: ['1k', '2k'], description: 'Optional Grok output resolution; defaults to 1k.' },
    ...
    async execute(args, exec): Promise<GeneratedValue> {
      ...
      if (active.provider === 'grok') {
        ...
        const generated = await generateGrokImage({ apiKey: credential.value, baseURL: active.baseURL, model: active.model,
          prompt: args.prompt, aspectRatio, resolution, maxBytes: ctx.attachments.imageLimits.maxImageBytes, signal: exec.signal })
        return saveGenerated(ctx, generated, active.provider, active.model, `${aspectRatio}, ${resolution}`, current(), exec, knownWorkspaceRoots)
      }
