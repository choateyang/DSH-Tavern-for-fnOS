...
import { generateGrokImage, GROK_RATIOS, GROK_RESOLUTIONS } from './grok.js'
...
export function studioProfile(config: Config, provider: CloudImageProvider, configured: boolean): StudioProviderProfile {
  ...
  if (provider === 'grok') {
    return { ...profile(provider, model, configured, GROK_RATIOS.map(option), GROK_RESOLUTIONS.map(value => ({ value, label: value })), '1:1', '1k'), supportsEditing: false }
  }
  if (provider === 'google') {
...
function providerConfig(config: Config, provider: CloudImageProvider, model?: string): Config {
  ...
  switch (provider) {
    case 'grok': return { ...config, provider, grokModel: model }
