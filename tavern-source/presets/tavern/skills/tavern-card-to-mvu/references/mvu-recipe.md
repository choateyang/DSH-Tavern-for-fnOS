...
[
  {
    ...
    "constant": false,
    "insertion_order": 100,
    "content": "{\"场景\":{\"地点\":\"入口\"},\"玩家\":{\"位置\":\"门口\"},\"人物库\":{\"$meta\":{\"extensible\":true,\"template\":{\"姓名\":\"\",\"设计\":{\"身份\":\"\",\"性格\":\"\",\"外貌\":\"\",\"稳定偏好\":\"\"},\"状态\":{\"位置\":\"未明确\",\"在场\":false,\"当前衣着\":\"未明确\"}}}}}",
    "extensions": {}
  },
  {
    "id": 1,
    ...
    "constant": true,
    "insertion_order": 100,
    "content": "只依据本轮已经发生的正文事实与当前变量快照更新状态。场景.地点和玩家.位置为字符串，仅在正文确认移动后改变；想去、准备去不算移动。人物库是当前对话专属、按姓名索引的可扩展对象。设计字段为身份、性格、外貌、稳定偏好，允许依据本卡设定合理创作；状态字段为位置、在场、当前衣着，只能依据正文。此示例不主动创建预备人物；若原卡本来有随机人物池，应在这里写明首次结算预先设计的数量和约束，尚未登场者设状态.在场为 false。新增时用 insert 提交符合模板的完整对象，登场和离场只更新状态并保留档案。用 mvu_submit_update 提交，路径相对于 stat_data，例如 /玩家/位置；无变化提交空 operations。不要输出 HTML 或变量协议文本。",
    "extensions": {}
  }
]
```
...
- `人物库` 的模板和规则属于后台变量协议，不进入前台正文上下文。实际人物保存在每个对话自己的 `stat_data` 中；新对话从空库重新开始。
- 只有规则明确列出的设计字段可以创作。`在场`、`位置`、当前衣着、关系进展和事件仍是状态字段；预先设计的人物用 `在场: false` 保存，状态视图默认过滤。
