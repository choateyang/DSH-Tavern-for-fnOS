...
func InitLogger() {
	...
	logFilePathVal = filepath.Join(globalPkgVar, "dsh-tavern.log")
...
func cleanOldBackups(dir string) {
	...
	for _, e := range entries {
		...
		name := e.Name()
		if (strings.HasPrefix(name, "dsh-tavern-") && strings.HasSuffix(name, ".log")) || strings.HasPrefix(name, "dsh-tavern.log.") {
...
func rotateLocked() {
	...
	backupPath := filepath.Join(dir, fmt.Sprintf("dsh-tavern-%s.log", nowStr))
	if _, err := os.Stat(backupPath); err == nil {
		backupPath = filepath.Join(dir, fmt.Sprintf("dsh-tavern-%s_%d.log", nowStr, time.Now().Nanosecond()/1000))
	}
