...
func main() {
	...
	runtimeDir = filepath.Join(globalDshHome, "runtime")
	tavernSourceDir = filepath.Join(globalDshHome, "apps", "dsh-tavern")

	InitLogger()
	LogInfo("DSH Tavern 服务初始化启动 (DATA_LIBRARY_PATH=%s, TRIM_APPDEST=%s, TRIM_APPVER=%s, DSH_RUN_USER=%s)", globalPkgVar, globalAppDest, globalAppVer, globalRunUser)
