...
func parsePluginCommand(input string) (*pluginCommand, error) {
	...
	if len(fields) < 2 || fields[0] != "dsh" || fields[1] != "plugin" {
		return nil, fmt.Errorf("请输入标准 dsh 命令，例如: dsh plugin --profile %s add 包名", tavernProfileName)
	}

	profile := tavernProfileName
	...
	if profile != tavernProfileName {
		return nil, fmt.Errorf("DSH Tavern 仅支持 %s Profile，不会操作 %s Profile", tavernProfileName, profile)
	}
	return cmd, nil
}
...
func pluginProfileDir() string {
	return filepath.Join(globalDshHome, "profiles", tavernProfileName)
}
...
func handleListPlugins(c *gin.Context) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			OK(c, pluginListPayload{Profile: tavernProfileName, Plugins: []pluginItem{}, Bundles: []string{}})
			return
		}
	...
	disabledMap, _ := ReadDisabledEntryMap(tavernProfileName)
	if disabledMap == nil {
	...
	if len(legacyDisabled) > 0 {
		for _, disName := range legacyDisabled {
			...
			entryIDs := ExtractPluginEntryIDs(tavernProfileName, disName)
			...
			if !isAlreadyDisabled {
				_ = SetPluginDisabled(tavernProfileName, disName, true)
				for _, eid := range entryIDs {
	...
	for _, name := range names {
		...
		entryIDs := ExtractPluginEntryIDs(tavernProfileName, name)
