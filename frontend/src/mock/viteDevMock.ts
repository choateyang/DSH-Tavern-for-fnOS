  ...
  return {
    ...
    configureServer(server) {
      ...
      const config = {
        ...
        data_library_path: '/vol1/@appdata/dsh-tavern',
      ...
      let plugins: PluginItem[] = [
        {
          ...
          author: 'DeepSeek AI',
          homepage: 'https://github.com/flizzywine/dsh-tavern',
          license: 'MIT',
      ...
      let logs: string[] = [
        '2026-08-21 16:00:00 [INFO] 启动 DSH Tavern 守护服务',
        '2026-08-21 16:00:01 [INFO] 服务主进程已拉起 (PID=1043416)，正在等待 Web 服务就绪...',
        '2026-08-21 16:00:03 [INFO] Web 服务就绪探测通过，反向代理启动完成 [0.0.0.0:3082 → http://127.0.0.1:3081]',
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (path.endsWith('/api/action') && req.method === 'POST') {
          ...
          if (action === 'start') {
            ...
            appendLog('[INFO] 正在启动 DSH Tavern 应用进程...')
            broadcast('status', getStatusPayload())
          ...
          if (action === 'restart') {
            ...
            appendLog('[INFO] 正在停止 dsh.tavern 应用进程...')
            broadcast('status', getStatusPayload())
        ...
        if (path.endsWith('/api/plugins/preview') && req.method === 'POST') {
          ...
          if (parts[0] !== 'dsh' || parts[1] !== 'plugin') {
            return sendJson(res, 0, 'success', {
              valid: false,
              ok: false,
              reason: '请输入标准 dsh 命令，例如: dsh plugin --profile tavern add 包名'
            })
          }
