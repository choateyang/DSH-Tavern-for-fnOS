              ...
              <n-layout-sider bordered :width="240" :native-scrollbar="false" class="hidden sm:block select-none z-10"
                ...
                <div class="flex flex-col gap-3 p-3">
                  ...
                  <n-tooltip trigger="hover" placement="right">
                    ...
                    <span>{{ appHasUpdate ? `发现 fnOS 应用包 v${appRemoteVersion ? appRemoteVersion.replace(/^v/, '') : ''}，前往 GitHub Releases 下载` : '访问 fnOS 应用包仓库' }}</span>
                  </n-tooltip>
...
const tabLabels: Record<TabKey, string> = {
  overview: '概览 · DSH Tavern',
  workspace: '工作区 · DSH Tavern',
  plugins: '插件管理 · DSH Tavern',
  snapshots: '快照管理 · DSH Tavern',
  logs: '运行日志 · DSH Tavern',
  settings: '应用设置 · DSH Tavern'
}
...
watch(tab, (newTab) => {
  trimSdk.setTitle(tabLabels[newTab] || 'DSH Tavern')
}, { immediate: true })
...
function openRepo() {
  const url = appHasUpdate.value
    ? 'https://github.com/choateyang/DSH-Tavern-for-fnOS/releases'
    : 'https://github.com/choateyang/DSH-Tavern-for-fnOS'
