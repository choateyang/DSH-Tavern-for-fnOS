                  ...
                  <template #label>
                    <div class="flex items-center gap-1.5">
                      ...
                      <n-tooltip :trigger="isTouch ? 'click' : 'hover'">
                        ...
                        DSH Tavern 本地服务进程监听端口，默认 3081
                      </n-tooltip>
                    </div>
                  </template>
                  <n-input-number v-model:value="config.server_port" :min="1" :max="65535" placeholder="3081"
                    class="w-full" />
                </n-form-item>
              </n-gi>
                  ...
                  <template #label>
                    <div class="flex items-center gap-1.5">
                      ...
                      <n-tooltip :trigger="isTouch ? 'click' : 'hover'">
                        ...
                        对外暴露的代理访问端口 (默认 3082)，用于 Web 客户端直连
                      </n-tooltip>
                    </div>
                  </template>
                  <n-input-number v-model:value="config.proxy_port" :min="1" :max="65535" placeholder="3082"
                    class="w-full" />
                </n-form-item>
              </n-gi>
                  ...
                  <template #label>
                    <div class="flex items-center gap-1.5">
                      ...
                      <n-tooltip :trigger="isTouch ? 'click' : 'hover'">
                        ...
                        点击概览页「进入 Tavern」时跳转的绝对 URL (例如 https://dsh.nas.com:3082)
                      </n-tooltip>
                    </div>
                  </template>
                  <n-input v-model:value="config.reverse_proxy_url" placeholder="例如 https://dsh.example.com:3082"
                    clearable />
                </n-form-item>
              </n-gi>
...
async function handleSave() {
  ...
  if ((isServerPortChanged.value || isHeapMemoryChanged.value || isProxyDshChanged.value) && systemStore.isRunning) {
    ...
    if (isServerPortChanged.value) {
      changes.push(`内部监听端口已由 ${savedConfig.value?.server_port || 3081} 变更为 ${config.value.server_port}`)
    }
    ...
    dialog.warning({
      title: '确认保存并重启核心服务？',
      content: `检测到${changes.join('，')}。保存设置后，系统将自动重启 DSH Tavern 后端进程以应用新配置。当前所有正在执行的任务可能会短暂中断，是否确认继续？`,
      positiveText: '保存并重启',
      negativeText: '取消',
