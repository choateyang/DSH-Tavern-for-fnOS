<template>
  <div v-auto-animate class="w-full flex-1 flex flex-col gap-4 sm:gap-6">
      ...
      <n-card :bordered="false" class="shadow-sm rounded-2xl">
        <div class="flex flex-col py-1 sm:py-2">
          ...
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <n-button type="primary" size="medium" :disabled="!isRunning" @click="systemStore.openTavernApp"
    ...
    <!-- 发现新版本升级确认弹窗 -->
    <n-modal v-model:show="showUpdateModal" preset="dialog" type="info" title="发现 DSH Tavern 更新" positive-text="立即更新"
      ...
      <div class="space-y-3 pt-1 text-xs leading-relaxed text-slate-600 dark:text-slate-300">
        ...
        <div class="p-3 rounded-xl bg-amber-500/[0.08] dark:bg-amber-500/[0.15] border border-amber-500/30 space-y-2">
          ...
          <div class="text-[11.5px] leading-relaxed text-amber-800 dark:text-amber-300">
            此操作只更新 GitHub 上的 DSH Tavern 程序源码，不更新 fnOS 应用包，也不单独升级 DSH 运行时。建议更新前创建快照以备回滚。
          </div>
          <div
...
const actionCards = computed<ActionCard[]>(() => [
  showStopCard.value
    ...
    : {
      ...
      desc: isStarting.value ? '正在拉起服务主进程并等待就绪…' : '拉起 DSH Tavern 后台核心服务',
  ...
  },
  {
    ...
    desc: '检查 GitHub 上游 DSH Tavern 提交，在线更新 Tavern 程序',
  ...
  },
  {
    ...
    icon: Tools,
    label: '重新部署',
    desc: '清空依赖缓存，重新部署 Tavern 与独立 DSH 运行时',
    ...
    confirmText: '此操作将清理本地依赖缓存，并重新部署 Tavern Profile 与独立 DSH 运行时；不会更新 fnOS 应用包。确定继续？'
  }
])
...
const confirmUpgrade = async () => {
  ...
  if (res.success) {
    message.success(res.message || '已开始拉取 DSH Tavern 更新并部署')
  } else {
