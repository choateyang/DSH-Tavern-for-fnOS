...
export const useSystemStore = defineStore('system', () => {
  ...
  const statusLabel = computed(() => {
    ...
    if (isBuilding.value) {
      if (activeAction.value === 'upgrade' || statusData.value.last_message?.includes('更新')) {
        return '更新中'
      }
      return '部署中'
    }
  ...
  async function openTavernApp(): Promise<void> {
    ...
    if (mode === 'port') {
      const port = cfg?.proxy_port || 3082
      await trimSdk.openURL(`https://${window.location.hostname}:${port}/`, '_blank')
      return
    }

    // 默认飞牛网关直连模式
    const gatewayUrl = `${window.location.origin}/app/dsh-tavern/fngateway/`
