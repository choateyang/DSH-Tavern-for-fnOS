...
func RestoreSnapshot(id string) error {
	...
	if !isRuntimeReady() {
		...
		if _, err := os.Stat(tarPath); err == nil {
			LogInfo("检测到内置 DSH Tavern 离线包，优先解压并安装: %s", tarPath)
			_ = safeRemoveAll(tavernSourceDir)
			if err := extractTarGz(tarPath, tavernSourceDir); err != nil {
				LogWarning("解压内置 Tavern 离线包失败 (%s): %s", tarPath, err)
			} else if err := installTavern(false); err != nil {
				LogWarning("快照还原自愈安装 Tavern 失败: %s", err)
			} else {
				refreshVersion()
				LogInfo("内置 DSH Tavern 运行环境恢复就绪")
			}
		} else {
			LogInfo("未检测到内置离线包，通过 Tavern 安装器在线恢复运行环境...")
